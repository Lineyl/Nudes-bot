...
class Repo:
    ...
    async def list_users(self, args=None):
        users = self.conn.users.find(args if args else {})
        users = await users.to_list(length=50000)
        return users

    async def list_for_top_users(self):
        users = self.conn.users.find({"status": 1, "stats.summary.profit_from_purchases": {"$gt": 0}})
        users = await users.to_list(length=50000)
        return users

    async def list_for_all_users(self):
        users = self.conn.users.find({"status": 1, "balance": {"$gt": 0}})
        users = await users.to_list(length=50000)
        return users
